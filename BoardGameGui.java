...
public Scene getScene2() {
	...
	// creates 40 squares and adds it to the list for later iteration	
		for (int i = 0 ; i < 40 ; i++) {
			...
			square.setPrefSize(screenSize.getWidth() * 0.075, screenSize.getHeight() * 0.13);
	...
	// adds all the named squares onto the board/gridpane
		...
		Scene scene2 = new Scene(gridBoard, screenSize.getWidth(), screenSize.getHeight());
		return scene2;
	}
	...
	// The action that occurs after number of players is picked		
		public void placePlayers() {
			...
			if (players.size() == 1) {
				...
				propertyList1= new Label (players.get(0).getName()+ " doesn't own any \nproperties yet");
				...
				propertyList1= new Label (players.get(0).getName()+ " doesn't own any \nproperties yet");
				propertyList2= new Label (players.get(1).getName()+ " doesn't own any \nproperties yet");
				balanceLabel1 =new Label("balance: 1500");
				...
				propertyList1= new Label (players.get(0).getName()+ " doesn't own any \nproperties yet");
				propertyList2= new Label (players.get(1).getName()+ " doesn't own any \nproperties yet");
				propertyList3= new Label (players.get(2).getName()+ " doesn't own any \nproperties yet");
				balanceLabel1 =new Label("balance: 1500");
				balanceLabel2 = new Label("balance: 1500");
				...
				propertyList2= new Label (players.get(1).getName()+ " doesn't own any \nproperties yet");
				propertyList3= new Label (players.get(2).getName()+ " doesn't own any \nproperties yet");
				propertyList4= new Label (players.get(3).getName()+ " doesn't own any \nproperties yet");
				balanceLabel1 =new Label("balance: 1500");
				balanceLabel2 = new Label("balance: 1500");
				balanceLabel3 =new Label("balance: 1500");
	...
	public class rollHandle implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			if (players.get(playersTurn).getJail()==true && jailYesNo==false){
				...
				turnMessage.setText("You are in jail. Would you like to \npay 100$ to get out, or roll for doubles?");
				Buy.setText("Pay");
				Roll.setText("Roll");
				jailYesNo=true;
				...
				coordinateVariable=move.move3(players.get(playersTurn));
					...
					turnMessage.setText(players.get(playersTurn).getName()+" passed go, Collect 200$ \nClick Ok to continue");
									actionCount++;
									buyCount=move.coordinate2(players.get(playersTurn));
				coordinateVariable=move.move3(players.get(playersTurn));
				}
					...
					turnMessage.setText(players.get(playersTurn).getName()+" goes to jail. \nRoll doubles to get out next time. \nClick roll to continue");
					//player.setJailcount();
					
					turnUpdate();
				}
				else if (messageCount==1){
					turnMessage.setText(players.get(playersTurn).getName()+" is just visiting jail \n Click Ok to continue ");
					actionCount++;
					buyCount=move.coordinate2(players.get(playersTurn));
					coordinateVariable=move.move3(players.get(playersTurn));
				}
			...
			//	coordinateVariable=move.move3(players.get(playersTurn));
				...
				if (coordinateVariable==1){
					...
					turnMessage.setText(players.get(playersTurn).getName()+" has landed at \n" + move.getOwner() +"'s property.\n They will pay them "+ board.get(buyCount-1).getRent()+ "\n Press Ok to continue");
				...
				//	buyCount=move.coordinate2(players.get(playersTurn));
					...
					//buyCount=move.coordinate2(players.get(playersTurn));
					turnMessage.setText(players.get(playersTurn).getName()+"  has landed at " + board.get(buyCount-1).getName()+ ". \nBuy for " + board.get(buyCount-1).getCost() +" ?");
					...
					turnMessage.setText("landed on Specialty Property");
					actionCount=0;
					coordinateVariable=0;
					turnUpdate();
				}
				...
				turnMessage.setText(players.get(playersTurn).getName() + " has purchased " +board.get(buyCount-1).getName()+ " \nfor " + board.get(buyCount-1).getCost());
				buyCount=-1;
				turnUpdate();
				actionCount=0;
				}
	...
	public class buyHandle implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			if(yesNo==true){
				turnMessage.setText("aight don't then");
				turnUpdate();
